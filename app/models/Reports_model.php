...
class Reports_model extends CI_Model
{
    ...
    public function AllStoresProfitLossDetails($startDate, $endDate)
    {
        $allStores = $this->db->get('tec_stores');
        foreach ($allStores->result() as $storeInfo){
            // skipping test
            if($storeInfo->id == 1)
                continue;
            // sales query
            $this->db->select("SUM(total) as total, DATE(date) as date");
            $this->db->where(['store_id' => $storeInfo->id]);
            $this->db->where(["DATE(date) >=" => date('Y-m-d', strtotime($startDate))]);
            $this->db->where(["DATE(date) <=" => date('Y-m-d', strtotime($endDate))]);
            $this->db->order_by('date', 'DESC');
            $this->db->group_by('date');
            $responseData[$storeInfo->name]['sales'] = $this->db->get('tec_sales')->result();
            ...
            $this->db->order_by('date', 'DESC');
            $this->db->group_by('date');
            $responseData[$storeInfo->name]['expenses'] = $this->db->get('tec_expenses')->result();
            ...
            $this->db->order_by('date', 'DESC');
            $this->db->group_by('date');
            $responseData[$storeInfo->name]['purchases'] = $this->db->get('tec_purchases')->result();
        }
