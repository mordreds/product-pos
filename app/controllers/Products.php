...
class Products extends MY_Controller
{
    ...
    function index() {
        ...
        // get store worth
        if (is_null($this->input->get('store_id', TRUE))) {
            $arr = [];
            foreach ($this->data['stores'] as $storeData) {
                $arr[] = $this->products_model->getTotalCostOfItems($storeData->id);
            }
            $res = array_sum($arr);
        }
        else {
            $res = $this->products_model->getTotalCostOfItems($this->input->get('store_id', TRUE));
        }

        $this->data['totalProductCost'] = number_format($res, 2);
